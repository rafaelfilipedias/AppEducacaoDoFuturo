...
namespace AppEducaçãoDoFuturo
{
    class ClassAluno
    {
        ...
        public string Curso_codCurso { get; set; }

        //Criando o objeto da classe de conexão como banco de dados
        ClassAcessoBD bd = new ClassAcessoBD();

        //Método para inserir Aluno com o parametro do Código do Usuário que irá realizar o cadastro
        public bool InserirAluno(string codUsuarioLogado)
        {
            try
            {
                //Conectando o banco 
                bd.Conectar();
                //Executando o INSERT
                bd.ExecutarComandosSql(String.Format("INSERT INTO Aluno  VALUES ('{0}','{1}','{2}','{3}','{4}','{5}','{6}','{7}','{8}')",
                    CodAluno, CodMatricula, Nome, Cpf, Telefone, Email, DataNasc, codUsuarioLogado, Curso_codCurso ));
        ...
        //Método para editar Aluno com o parametro do Código do Aluno que irá ser editado
        public bool Editar(string codAlunoEditar)
        {
            try
            {
                ...
                bd.Conectar();
                //Executando o UPDATE
                bd.ExecutarComandosSql(String.Format("UPDATE Aluno SET nome = '{0}', telefone = '{1}', email = '{2}', dataNasc = '{3}', Curso_codCurso= '{4}' WHERE codAluno = '{5}'", Nome,Telefone, Email, DataNasc, Curso_codCurso, codAlunoEditar));
                //Desconectando o banco
                bd.Desconectar();
        ...
        //Método para excluir Aluno com o parametro do Código do Aluno que irá ser excluido
        ...
        public DataTable RetAlunoBusca(string cod)
        {
            //Conectando com o banco 
            bd.Conectar();
            //Objeto que recebe o SELECT para buscar por código ou cpf do aluno
